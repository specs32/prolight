...
const int BUTTON_DOWN = 13;          // define the other button pin PC7
...
void VOLTCHECK(){
  ...
  blank();   
  //digitalWrite(RED, HIGH);    // enable 
  int  mv = readVcc();
  u8g2.clearBuffer();  
  u8g2.setFont(u8g2_font_helvB12_te); // u8g2_font_ncenB08_tr
  //u8g2.setCursor(0, 15);
  //u8g2.print(mv);

  u8g2.drawStr(10,15,"AllPCB");  // write something to the internal memory
...
void PLASMA() {
  ...
  float phase = 0.0;
  float phaseIncrement = 0.001;  // Controls the speed of the moving points. Higher == faster. I like 0.08 .
  float colorStretch = 0.38; 
...
//  int16_t temp;
  ...
  //Acelerometer axis X
  myIMU.readRegisterInt16(&ax, LSM6DS3_ACC_GYRO_OUTX_L_XL);
  
//  Serial.print(" X = ");
//  Serial.println(accelx);  
  
  //Acelerometer axis Y 
  myIMU.readRegisterInt16(&ay, LSM6DS3_ACC_GYRO_OUTY_L_XL);
  
//  Serial.print(" Y = ");
//  Serial.println(accely);
  
  //Acelerometer axis Z  
  myIMU.readRegisterInt16(&az, LSM6DS3_ACC_GYRO_OUTZ_L_XL);
...
//  Serial.println(accelz);
  ...
  for( row=0; row<7; row++ ) {
    ...
    for( col=0; col<9; col++ ) {
      ...
      //strip.setPixelColor(col + (9 * row), strip.Color(color_1, color_2, color_3));


      value.b = abs(color_3 - (ax / 100));
      value.g = abs(color_2 - (ay / 100));
      value.r = abs(color_1 - (az / 100));

      int n = (col + (9 * row));

      PIXEL.set_crgb_at(n, value);      
